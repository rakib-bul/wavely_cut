...
export default function Sidebar({
  ...
  const getRoleBadgeColor = (role: UserRole) => {
    switch (role) {
      ...
      case "manager": return "bg-slate-500/10 text-slate-700 border-slate-500/20";
  ...
  return (
    <aside className="w-64 bg-white text-slate-700 flex flex-col h-screen sticky top-0 shrink-0 border-r border-slate-250 font-sans transition-colors duration-200">
      ...
      <div className="p-6 flex items-center gap-3">
        ...
        <div>
          ...
          <span className="text-[9px] uppercase tracking-widest text-slate-500 font-semibold block mt-1">
      ...
      <div className="p-4 mx-4 mb-4 bg-slate-50 rounded-xl border border-slate-200">
        <label className="text-[9px] uppercase tracking-wider text-slate-500 font-bold block mb-1.5 flex items-center gap-1">
          <Fingerprint size={10} className="text-slate-600" /> Simulated User (Auth / RLS)
        </label>
        ...
        <select
          ...
          className="w-full bg-white border border-slate-200 rounded-md text-xs py-1.5 px-2 text-slate-800 font-medium focus:ring-1 focus:ring-slate-900 outline-none"
        >
          {profiles.map(p => (
        ...
        <div className="mt-2.5 flex items-center justify-between gap-1">
          ...
          <button
            onClick={onLogout}
            className="text-[10px] text-rose-600 hover:underline bg-transparent border-none cursor-pointer font-semibold"
          >
            Logout
          </button>
        </div>
        ...
        {navItems.map((item) => {
          ...
          if (!hasAccess) {
            return (
              <div 
                key={item.id} 
                className="flex items-center gap-3 text-slate-300 px-3 py-2 rounded-md text-xs font-medium cursor-not-allowed opacity-40 select-none"
      ...
      {/* Footer Settings */}
      <div className="p-4 border-t border-slate-200 flex items-center mx-2">
        <div className="flex items-center space-x-2">
          <UserSquare2 size={16} className="text-slate-400" />
          <span className="text-[11px] text-slate-600 max-w-[145px] truncate" title={currentProfile.email}>
